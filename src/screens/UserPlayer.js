...
function UserPlayer({route}) {
  ...
  trackArtwork,setTrackArtwork,songId,setSongId
  }=useAudioContext()
  ...
  const setupPlayer = async () => {
    ...
    // let inputString = route.params?.id
    // console.log(inputString,'bol')
    //   let response = await axios.get(`https://geetglow.com/api/songs/${inputString}`)
    //    musicData = response.data
    //    console.log(musicData,"hii")
    //   // Prepare the tracks array
    //   let tracks = musicData.map(item => ({
    //     id: item.id, // Replace with actual id field
    //     url: 'https://geetglow.com/'+item.url, // Replace with actual url field
    ...
    const gettrackdata = async (tracks) => {
    let trackIndex = await TrackPlayer.getActiveTrackIndex();
  ...
  useTrackPlayerEvents(["playback-track-changed"], async event => {
    ...
    const data=async()=>{
        let inputString = route.params?.id
        // console.log(inputString,'bol')
         setSongId(inputString)
        //  console.log(songId,inputString,'noteqal')
         if(songId!==inputString||songId===undefined){
            setSongId(songId)
            let response = await axios.get(`${process.env.LINK}/songs/${inputString}`)
            // console.log(response.data,'hii')

            try{
              let tracks = response.data.map(item => ({
                  id: item.id, // Replace with actual id field
                  url: 'https://geetglow.com/'+item.url, // Replace with actual url field
                  title: item.song_name, // Replace with actual title field
                  artist: item.artist_name, // Replace with actual artist field
